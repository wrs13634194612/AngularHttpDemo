...
/*  onUpdateClick(key: string): void {
  ...
  onUpdateClickTwo(key: string): void {
    const dialogRef = this.dialog.open(DialogContentExampleDialog, {
      data: {url:'https://www.baidu.com',id:key},
      maxWidth: '100vw',
    ...
    dialogRef.afterClosed().subscribe((result:Todo)=>{
      ...
      /* 10086 ：字符串五位表示新增   其他表示 修改*/

      if (key.length>4){
        this.service
          .postAddShop(key,result)
          .pipe(tap())
          .subscribe((resp) => {
            console.log(resp)
          });
